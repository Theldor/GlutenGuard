      ...
      <div className="flex-1 overflow-auto px-4 pt-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-[13px] text-[#846848]">Sensitivity: {severityLabels[sev]}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2 mb-6">
          ...
          <button
            onClick={() => {}}
            className="flex flex-col items-center justify-center gap-2 px-4 py-6 rounded-2xl border-2 border-[#dbcdbd] bg-white text-left hover:bg-[#f9ebd2]"
          >
        ...
        <div className="space-y-2 mb-6">
          ...
          <div className="border border-[#dbcdbd] rounded-2xl overflow-hidden">
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-4 text-left bg-white hover:bg-[#f9ebd2]"
            >
              <div className="flex items-center gap-3">
                <AlertCircle size={20} className="text-[#423424]" />
                <span className="text-[#100d09] text-[15px]" style={{ fontWeight: 500 }}>My Restrictions</span>
              </div>
              <ChevronDown 
                size={20} 
                ...
                <div className="bg-[#fcf5e9] border border-[#dbcdbd] rounded-2xl p-4 mb-3 mt-3">
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      <CheckCircle size={20} className="text-[#6d7854]" />
                      <span className="text-[#100d09] text-[16px]" style={{ fontWeight: 600 }}>Celiac Disease</span>
                    </div>
                  ...
                  <p className="text-[13px] text-[#846848] ml-7">Primary condition (locked)</p>
                </div>
                ...
                <div className="space-y-2">
                  {restrictions.map((restriction) => (
                    <div
                      ...
                      className="bg-white border border-[#dbcdbd] rounded-2xl px-4 py-4"
                    >
                      <span className="text-[#100d09] text-[15px]">{restriction.name}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="bg-[#fcf5e9] border border-[#dbcdbd] rounded-2xl p-4 mt-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[13px] text-[#846848] mt-0.5">Moderate reaction</p>
                    </div>
                    <button className="p-2">
                      <Edit2 size={18} className="text-[#423424]" />
                    </button>
                  </div>
                </div>
              </div>
              ...
              <div className="px-4 pb-4 border-t border-[#dbcdbd]">
                ...
                <div className="space-y-2">
                  {customIngredients.map((ingredient, index) => (
                    <div
                      key={index}
                      className="bg-white border border-[#dbcdbd] rounded-2xl px-4 py-3.5 flex items-center justify-between"
                    >
                      <span className="text-[#100d09] text-[15px]">{ingredient}</span>
                      <button
                        onClick={() => removeCustomIngredient(index)}
                        className="p-1"
                      >
                        <X size={18} className="text-[#423424]" />
                      </button>
                    </div>
                  ))}
                </div>
          ...
          {menuItems.map((item) => (
            <button
              key={item.label}
              onClick={item.action}
              className="w-full flex items-center justify-between px-4 py-4 rounded-2xl border border-[#dbcdbd] text-left bg-white hover:bg-[#f9ebd2]"
            >
              <div className="flex items-center gap-3">
