    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 overflow-auto px-4 py-4">
        <div className="flex items-center gap-2 mb-1">
          <h2 className="text-[#100d09]">Senza Gluten</h2>
          <span className="px-2 py-0.5 bg-[#f3f5f0] text-[#525a3f] rounded text-[12px]">Italian</span>
          <span className="text-[13px] text-[#846848]">0.3 mi</span>
        </div>

        <div className="bg-[#e7eae1] rounded-lg p-3 mt-3 mb-4">
          <p className="text-[#6d7854] text-[14px]" style={{ fontWeight: 600 }}>Celiac-Appropriate</p>
          <p className="text-[13px] text-[#6d7854] mt-1">
        ...
        <Section title="What celiac diners say">
          {reviews.map((r) => (
            <div key={r.name} className="bg-[#fcf5e9] rounded-lg p-3 mb-2">
              <div className="flex items-center gap-2 mb-1">
                <div className="flex">
                  {Array.from({ length: r.rating }).map((_, i) => (
                    <Star key={i} size={12} className="text-[#fac905] fill-[#fac905]" />
                  ))}
                </div>
                <span className="text-[12px] text-[#846848]">{r.severity}</span>
              </div>
              <p className="text-[14px] text-[#100d09]" style={{ fontWeight: 500 }}>{r.name}</p>
              <p className="text-[13px] text-[#423424] mt-1">{r.text}</p>
            </div>
          ))}
        </Section>
        ...
        <Section title="Menu highlights">
          {menuHighlights.map((m) => (
            <div key={m.name} className="flex justify-between items-center bg-[#fcf5e9] rounded-lg p-3 mb-2">
              <div>
                <p className="text-[14px] text-[#100d09]">{m.name}</p>
                <p className="text-[12px] text-[#846848]">{m.desc}</p>
              </div>
              <span className="px-2 py-0.5 rounded text-[11px] bg-[#e7eae1] text-[#6d7854]">{m.risk}</span>
            </div>
          ))}
        </Section>

        <Section title="Before you go">
          <p className="text-[14px] text-[#100d09] mb-2" style={{ fontWeight: 500 }}>Check with the restaurant:</p>
          <div className="flex gap-2 mb-4">
            <button 
              onClick={() => nav("/explore/contact")}
              className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-[#f3f5f0] text-[#525a3f] rounded-lg text-[14px]"
            ...
            <a 
              href={phoneNumber}
              className="flex items-center justify-center gap-2 px-4 py-2.5 border border-[#dbcdbd] rounded-lg text-[#100d09] text-[14px]"
              style={{ fontWeight: 500 }}
            >
              <Phone size={16} /> Call
            </a>
          ...
          <p className="text-[14px] text-[#100d09] mb-2" style={{ fontWeight: 500 }}>Questions to ask when you arrive:</p>
          <ol className="list-decimal list-inside text-[13px] text-[#423424] space-y-1 mb-4">
            <li>Can you confirm you have a dedicated GF kitchen?</li>
            <li>Are your GF pasta dishes prepared separately?</li>
          </ol>
          <p className="text-[14px] text-[#100d09] mb-2" style={{ fontWeight: 500 }}>Reservation note:</p>
          <div className="bg-[#fcf5e9] rounded-lg p-3 flex items-start gap-2">
            <p className="text-[13px] text-[#423424] flex-1">
              One guest has celiac disease and requires gluten-free preparation with no cross-contamination.
            </p>
            <button className="shrink-0 text-[#525a3f]"><Copy size={16} /></button>
          </div>
        </Section>
      </div>
      ...
      <div className="border-t border-[#dbcdbd] px-4 py-3 space-y-3 bg-white">
        ...
        <div className="flex gap-3">
          <button 
            className="flex-1 flex items-center justify-center gap-2 py-3 border border-[#dbcdbd] rounded-lg text-[#100d09] text-[15px]"
            style={{ fontWeight: 500 }}
          >
            <MessageCircle size={16} /> Chat
          </button>
          <button className="flex items-center justify-center gap-2 px-4 py-3 border border-[#dbcdbd] rounded-lg text-[#100d09]">
            <Bookmark size={18} />
          </button>
          <button className="flex items-center justify-center gap-2 px-4 py-3 border border-[#dbcdbd] rounded-lg text-[#100d09]">
            <Share2 size={18} />
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-5">
      <p className="text-[#100d09] mb-2" style={{ fontWeight: 600 }}>{title}</p>
      {children}
    </div>
  );
}